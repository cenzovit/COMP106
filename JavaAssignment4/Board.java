...
public class Board extends JComponent {
  ...
  protected Tile changeShapeHelper(Tile selectedTile, String shape){
    int xInd = selectedTile.getRowIndex();
    int yInd = selectedTile.getColIndex();
    Tile newTile;
    if (shape == "Rectangle"){
      newTile = new rectangleZeroTile(xInd, yInd, colors[0], this);
    }
    else if (shape == "Triangle"){
      newTile = new triangleOneTile(xInd, yInd, colors[1], this);
    }
    else if (shape == "Circle"){
      newTile = new circleTwoTile(xInd, yInd, colors[2], this);
    }
    else if (shape == "Diamond"){
      newTile = new diamondThreeTile(xInd, yInd, colors[3], this);
    }
    else if (shape == "Pentagon"){
      newTile = new pentagonFourTile(xInd, yInd, colors[4], this);
    }
    else{
      newTile = new octagonFiveTile(xInd, yInd, colors[5], this);
    }	
    newTile.setSelected(true);
    gameBoard[xInd][yInd] = newTile;
    return newTile;
  }

  protected void changeColor(Color newColor, int tileValue){
    colors[tileValue] = newColor;
    changeColors();
  }
  ...
  private Tile randomTile(int xInd, int yInd){
    ...
    if (randTile == 0){
      ...
      return new diamondThreeTile(xInd, yInd, colors[3], this);
    }
    else if (randTile == 4){
      return new pentagonFourTile(xInd, yInd, colors[4], this);
    }
    else{
      return new octagonFiveTile(xInd, yInd, colors[5], this);
    }
  }
}
